...
export const wagmiConfig = createConfig({
  ...
  connectors: [
    // Note: the standalone `metaMask()` connector pulls in
    // `@metamask/connect-evm` which fails to resolve in some mobile WebView
    // bundles ("Cannot find module '@metamask/connect-evm'"). MetaMask Mobile
    // exposes `window.ethereum` inside its in-app browser, so the standard
    // `injected()` connector handles it just as well — without that extra
    // dep.
